...
class SingleProductionSpaces extends React.Component {
    state = {
        page: [],
        loading: false,
        id:'',
    ...
    componentDidMount() {
        ...
        let slug  = this.props.location.pathname.replace('/production-space/','');

        let menuURL = process.env.REACT_APP_SITE_URL+"/wp-json/wp/v2/production_spaces?slug="+slug+"&_embed";
    ...
    render() {
        ...
        return (
            <div>
                <Header/>
                <h1>single PRODUCTION SPACES page</h1>
...
export default SingleProductionSpaces;